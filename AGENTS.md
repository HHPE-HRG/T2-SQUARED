...
- **T2-owned text.** This Forgejo fork governs only T2-owned paths. Unchanged upstream T3 prose is out of scope. Raw conversation fixtures stay byte-identical and are not rewritten. Run `npm run ci:asd-ste100` for the mechanical rule-subset checker. Run `npm run ci:work-registry` for T2_Squared-Work-Registry. Do not claim language-authority approval. See `docs/internals/asd-ste100-enforcement.md` and `docs/internals/t2-squared-work-registry.md`.
